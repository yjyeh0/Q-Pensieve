...
class SacAgent:
    ...
    def __init__(self, env, log_dir, state_mean, state_std,
        ...
        self.set_num = prefer_num  # set of �'
    ...
    def learn(self):
        ...
        for _ in range(self.set_num - 1):
            ...
            PREF_SET.append(p)
        #
        #   policy_loss, entropies = self.calc_policy_loss_new(batch, weights, preference, PREF_SET)
        # else:
    ...
    def calc_critic_loss(self, batch, weights, preference):
        ...
        qn_loss = [torch.mean(torch.tensordot((curr_q - target_q).pow(2), preference, dims=1) * weights) for curr_q in
                   curr_qn]
    ...
    def offline_calc_policy_loss(self, batch, weights, preference, PREF, is_online):
        ...
        sampled_action, entropy, _ = self.policy.sample(states, preference_batch)
        # yjyeh:
        sampled_action = sampled_action.unsqueeze(1).repeat(1, self.set_num, 1).flatten(0, 1)
        ...
        b_pref_entropy = entropy.repeat(1, self.set_num).reshape(
            -1)  # dim = 256*4  (e1, e1, e1, e1, e2, e2, e2, e2, e3, ...)
        ...
        for a, c in enumerate([self.critic] + self.Q_memory.sample()):  # Use critic from Q Replay Buffer
            ...
            l = - q - self.alpha * b_pref_entropy
            l = l.reshape(-1, self.set_num)  # [256, 4]
            losses.append(l)  # list[ 5, [256, 4]]

        losses = torch.hstack(losses)  # dim = [256, 20]
        ...
        if not is_online:
            ...
            policy_loss = -1 * torch.abs(policy_loss.detach()) * torch.mean(
                torch.nn.functional.cosine_similarity(actions.double(), action_batch.double()))
        return policy_loss, entropy

# Q_Pensieve new version
    def calc_policy_loss_new(self, batch, weights, preference, PREF):
        start = time.time()
        states, _, actions, rewards, next_states, dones = batch
        preference_batch = preference.repeat(self.batch_size, 1)
        sampled_action, entropy, _ = self.policy.sample(states, preference_batch)
        ...
        c_cnt = 0
        for a, c in enumerate([self.critic] + [self.critic, self.critic]):  # Use critic from Q Replay Buffer
            for b, i in enumerate(PREF):  # Get Q from preference set W
                ...
                qn = c(states, sampled_action, p_batch)  # q(s1,w1) q(s1,w2), q(s1,w3), q(s1,w4), q(s2,w1),...
        ...
        losses = torch.stack(losses, dim=1)  # 256,4*5
        policy_loss, idx = torch.min(losses, 1)  # 256
        # ll = idx.detach().cpu()[:, 0].tolist()
        policy_loss = torch.mean(policy_loss)

        # sampled_action, entropy, _ = self.policy.sample(states, preference_batch)  # entropy: 256,1
    ...
    def evaluate_(self, preference, ind):
        ...
        writer.add_scalar(
            "eval/tot_return" + np.array2string(preference, formatter={'float_kind': lambda x: "%.2f" % x}),
            np.dot(preference, mean_return), eval_ep)
