...
class MOPrioritizedMemory(MOMultiStepMemory):
    ...
    def __init__(self, capacity, state_shape, reward_shape, action_shape, device,
        ...
        super(MOPrioritizedMemory, self).__init__(
            capacity, state_shape, reward_shape, action_shape, device, gamma, multi_step)
        self.alpha = alpha
        self.beta = beta
        self.beta_annealing = beta_annealing
        self.epsilon = epsilon

    def append(self, state, preference, action, reward, next_state, done, error, episode_done=False):
    ...
    def calc_priority(self, error):
        # yjyeh: todo  lamda * error?
        tmp = np.sum(np.abs(error))
        # return (np.abs(error) + self.epsilon) ** self.alpha
        return (tmp + self.epsilon) ** self.alpha
    ...
    def reset(self):
        super(MOPrioritizedMemory, self).reset()
